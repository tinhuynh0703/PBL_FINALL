...
// import { QueryDateDto } from "../dto/requests/date-query.dto";
...
export class OrdersController {
  ...
  findByDate(
    ...
    console.log(param);
    return this.orderService.getListOrderByDateRange(param);
  }
  ...
  async findCartByUserId(@User() user): Promise<Order[]> {
    return this.orderService.findCartByUserId(user);
  }
  ...
  @ApiOperation({
    description: 'Add into cart',
    summary: 'Add into cart',
  })
  @Post('/cart')
  @ApiCreatedResponse({
    description: 'Add Into cart successfully',
    type: CreateOrderDto,
  })
  @ApiUnauthorizedResponse({
    description: 'Please log in',
  ...
  async addItemToCart(
    ...
    try {
      const order = await this.orderService.addCart(createOrderDto, user);

      if (user.deviceToken.length > 0) {
        const notification: PushNotificationDto = {
          deviceToken: user.deviceToken,
          title: TitleOrder,
          message: MessageAddNewCart,
        };
