...
export class OrdersService {
  ...
  async findByUserId(
    ...
    const idStatusCart = await this.statusService.findByName('cart')
    return this.orderModel
      .find({ $and: [{ user: user._id }, { orderStatus: { $ne: idStatusCart }}] })
  ...
  async findCartByUserId(
    ...
    const idStatusCart = await this.statusService.findByName('cart')
    return this.orderModel
      .find({ $and: [{ user: user._id }, { orderStatus: idStatusCart }] })
      .populate({ path: 'orderStatus', select: ['name', 'value'] })
      .populate({ path: 'product', select: ['images', 'name', 'price'] })
      .sort({ createdAt: 'desc' })
      .exec();
  ...
  async addCart(createOrderDto: CreateOrderDto, user: User): Promise<Order> {
    const freeUnit = user.freeUnit;
    const newFreeUnit = freeUnit - createOrderDto['quantity'];
    const statusInCart = await this.statusService.findByName('cart');

    const newOrderInCart = new this.orderModel({
      ...createOrderDto,
      product: createOrderDto.productId,
      quantityBilled: newFreeUnit < 0 ? -newFreeUnit : 0,
      user: user._id,
      orderStatus: statusInCart._id,
    });
    await newOrderInCart.save();
    ...
    return newOrderInCart.populate([
      { path: 'product', select: ['images', 'price', 'name'] },
      { path: 'orderStatus', select: ['value', 'name'] },
      { path: 'user', select: ['name', 'phoneNumber', 'avatarUrl'] },
    ]);
  ...
  getListOrderByDateRange(param) {
    const startAt: Date = new Date(param.from);
    const endAt: Date = new Date(param.to);
